...
import { invalidateUserPermissionsCache } from '../services/auth.service';
import { invalidateDocumentCache } from '../services/document.service';
...
export const cacheWarmers = {
  ...
  warmPermissions: async (userId: string) => {
    const { getUserPermissions } = await import('../services/auth.service.js');
    try {
  ...
  warmDocument: async (documentId: string, userId?: string) => {
    const { getDocument } = await import('../services/document.service.js');
    try {
