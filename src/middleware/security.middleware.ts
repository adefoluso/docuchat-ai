...
export const trackSuspiciousActivity = (req: Request, res: Response, next: NextFunction) => {
  ...
  const getFirstParamValue = (param: string | string[] | undefined): string | undefined => {
  ...
  const documentId = getFirstParamValue(req.params.documentId) || getFirstParamValue(req.params.id);
  if (documentId) {
